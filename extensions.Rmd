...
In these situations, we have observations clustered within both person and image, but person and image are not nested within one another- all participants see all 10 images.  Such a situation is typically referred to as one in which there are <span class="emph">crossed</span> random effects, which pretty much just means non-nested, or even more to the point, 'default'.  In the situations we'll look at next, we will have multiple sources variances to consider.
...
The above visualizations allows us to actually see less variability among students for secondary schools. Note that we have the usual extensions here if desired.  As an example, we could also do random slopes for student characteristics.
...
This represents a covariance structure of <span class="emph">compound symmetry</span>, where correlations, the off-diagonal values, are constant. It is the default in most mixed model settings, and the same as what is shown visually above. Now let's start to think about other types of covariance structures. 
...
A newer alternative to keep in mind is <span class="pack">glmmTMB</span>.  It would allow one to stay more in the <span class="pack">lme4</span> syntax style and provide similar looking output.
