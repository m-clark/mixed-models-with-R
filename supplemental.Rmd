...
The following information about priors assumes some background knowledge of Bayesian analysis, particularly for regression models. The Stan development group offers recommendations [here](https://github.com/stan-dev/stan/wiki/Prior-Choice-Recommendations), so refer to it often.  Note that Stan does not require conjugacy, unlike tools such as BUGS/JAGS.  This frees one up to use other prior distributions as they see fit. 

#### Fixed Effects

For fixed effect regression coefficients, normal and student t would be the most common prior distributions, but unfortunately the default brms (and rstanarm) implementation does not specify any, and so defaults to uniform/improper, which is a poor choice.  You will want to set this for your models. Note that scaling numeric predictors works here just like it does with lme4, and makes specifying the prior easier as well.

#### Random Effects



estimated, not predicted; show some stan code

Typically the priors for the variance components are half-t, with truncation at 0.
variance: t, historical
...
Random intercept and slope (for `x`).
...
brm(y ~ cs(x) + z + (1 | g),     # x has category specific effects
    family = acat)  
...
For multiple outcomes we can allow random effects to be correlated.  In the following, `ID1` is an arbitrary label that serves to connect the modeled random effects across multiple outcomes `y1` and `y2`.
...
This would make sense for zero-inflated models for example, where we want random effects for the same clustering to be correlated for both the count model and zero-inflated model.
