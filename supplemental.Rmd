...
We can allow the variance components themselves to vary by some group.  

```{r, eval=FALSE}
brm(count ~ Trt + (1|gr(g, by = Trt)))
```

Multi-membership models, where individuals may be long to more than one cluster can also be used.  In the following, g1 and g2 are identical conceptually, but may take on different values for some observations.
...
For multiple outcomes we can allow random effects to be correlated.  In the following, `ID1` is an arbitrary label that serves to connect the modeled random effects.
...
We can model similar situations where the functional form is known.
