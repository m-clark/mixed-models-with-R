...
# gridExtra::grid.arrange(gint2, gslopes2, ncol=2)
```
...
# gridExtra::grid.arrange(gint3, gslopes3, ncol=2)
```
...
# gridExtra::grid.arrange(gint4, gslopes4, ncol=2)
```


```{r all_together_now, echo=F, fig.align='left', out.width='100%', cache.rebuild=T}
# cowplot::plot_grid(gint, gint2, gint3, gint4,
#                    gslopes, gslopes2, gslopes3, gslopes4, 
#                    nrow=2)
gridExtra::grid.arrange(gint, gint2, gint3, gint4,
                   gslopes, gslopes2, gslopes3, gslopes4, 
                   nrow=2)
```
...
[^fe_comparison]: This is just one reason not to take a fixed effects approach. You would have to estimate 400 parameters, but without anything (inherently) to guard against overfitting.  The so-called 'fixed effects model' from the econometrics perspective gets around this by demeaning variables that vary within groups, i.e. subtracting the per group mean. This is equivalent to adding a dummy variable for the groups, though computationally more viable model is then fit.  But while it controls for group level effects, we still cannot estimate them. Traditional approaches to fixed effects models also do not have any way to incorporate random slopes.

```{r plm, echo=FALSE, eval=FALSE}
plm_model = plm::plm(gpa ~ occasion, model = 'within', data=gpa, index = c('student'), effect = "individual")
