...
<br>
Let us now assume that the trend over time are worth allowing to vary by student.  Using <span class="pack">lme4</span>, this is quite straightforward.
...
The slope variance for occasion might not look like much in comparison, but slopes are on a notably different scale than the intercept.  Note that the mean slope for the semester to semester effect, our fixed effect, is `r round(fixef(gpa_mixed)[2], 2)`, but from student to student it bounces around half that.  Thus we could expect most students to fall somewhere between a flat effect of zero to more than double the population average. 
