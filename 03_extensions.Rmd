...
In such a case we have observations clustered within both person and image, but person and image are not nested within one another- all participants see all 10 images.  Such a situation is typically referred to as one in which there are <span class="emph">crossed</span> random effects.  In such settings we have multiple sources variances to consider.
...
The following shows the difference in the results from treating ward as nested vs. crossed. What do you notice is different?
...
vc = tidy(VarCorr(gpa_vis_cov))
totvar = sum(vc$vcov)
tidy(VarCorr(gpa_vis_cov)) %>% 
  select(-var1, -var2) %>% 
  rename(variance=vcov, sd=sdcor) %>%  
  mutate_if(is.numeric, arm::fround, digits=3) %>% 
  pander()
...
Consider the following for an individual and three time points.
...
vc_nlme = (c(1.0000000, coef(heterovar_res$modelStruct$varStruct, unconstrained=F))*heterovar_res$sigma)^2
vc_compare = rbind(glmmTMB=vc_glmmtmb, nlme=vc_nlme) %>% 
  round(4)
colnames(vc_compare) = expand.grid('year', 1:3, 'sem', 1:2) %>% unite(col = year_sem, everything()) %>% pull()
...
pander(tidy(sd_model, 'fixed', conf.int=T) %>% mutate_if(is.numeric, arm::fround, digits=2), justify='lrrrrrr')
```
<br>
