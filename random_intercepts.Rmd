...
$$\mathrm{gpa} = b_{\mathrm{intercept}} + b_{\mathrm{occ}}\cdot \mathrm{occasion} + (\mathrm{effect}_{\mathcal{student}} + \epsilon)$$
...
$$\mathrm{gpa} = (b_{\mathrm{intercept}} + \mathrm{effect}_{\mathcal{student}}) + b_{\mathrm{occ}}\cdot \mathrm{occasion} +  \epsilon$$
Or more succinctly:
...
$$\mathrm{gpa} = b_{\mathrm{intercept}} + b_{\mathrm{occ}}\cdot \mathrm{occasion}+ b_{\mathrm{sex}}\cdot \mathrm{sex} + (\mathrm{effect}_{\mathcal{student}} + \epsilon)$$
...
[^vcmodel]: Actually, the simplest model would have no covariates at all, just <span class="emph">variance components</span>, with no correlations among the random effects.  Such a model can be interesting to look at while exploring your data, but would rarely suffice on its own to tell the story you desire to.
