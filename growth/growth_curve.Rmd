...
In the above, $Y$ is our observed variable, $b_{intercept}$ is the intercept as in a standard linear regression model, $\lambda$ is the coefficient (<span class="emph">loading</span> in factor analysis/SEM terminology) regarding the effect ot the latent variable, represented as $F$. The latent variable is assumed normally distributed, with zero mean, and some estimated variance, just like the random effects in mixed models.  
...
$$\mathcal{GPA} = b_{\mathrm{int\_student}} + b_{occ}*\mathrm{time} + \epsilon $$

$$b_{\mathrm{int\_student}} = b_{\mathrm{intercept}} + \mathrm{re}_{\mathrm{intercept}}$$   

Plugging in becomes:

$$\mathcal{GPA} = b_{\mathrm{intercept}} + b_{occ}*\mathrm{occasion} +  \mathrm{re}_{\mathrm{intercept}} + \epsilon $$

*subject level covariate added*

$$b_{\mathrm{int\_student}} = b_{\mathrm{intercept}} + b_{sex}*\mathrm{sex} + \mathrm{re}_{\mathrm{intercept}}$$ 

But if we plug that into our level 1 model, it just becomes:

$$\mathcal{GPA} = b_{\mathrm{intercept}} + b_{sex}*\mathrm{sex} + b_{occ}*\mathrm{occasion} +  \mathrm{re}_{\mathrm{intercept}} + \epsilon $$
...
$$\mathcal{GPA} = b_{\mathrm{int\_student}} + b_{\mathrm{occ\_student}}\cdot \mathrm{occasion} + \epsilon$$
$$b_{\mathrm{int\_student}} = b_{\mathrm{intercept}} + b_{sex}*\mathrm{sex} + \mathrm{re}_{\mathrm{intercept}}$$ 
$$b_{\mathrm{occ\_student}} = b_{\mathrm{occ}} + \gamma*\mathrm{sex} + \mathrm{re}_{\mathrm{occasion}}$$


And after plugging in:

$$\mathcal{GPA} = \color{#b2001d}{b_{\mathrm{intercept}} + b_{sex}*\mathrm{sex} + b_{occ}*\mathrm{occasion} + \mathbf{\gamma*\mathrm{sex}*\mathrm{occasion}}}  + \color{#001eb2}{\mathrm{re}_{\mathrm{intercept}} + \mathrm{re}_{\mathrm{occasion}}*\mathrm{occasion}} + e$$
...
Similarly, if we had a <span class="emph">time varying covariate</span>, average weekly hours spent in the library, it'd look like the following.  The gpa data doesn't really come with a useful time-varying covariate, so I've added one for this demo.
...
With time varying covariates, i.e. those that can have a different value at each time point, the syntax starts to get tedious.  Here we add `lib_hours`, but we need to convert it to wide format and add it to our previous data.
...
Note the problem here is similar to that with the residual variances. Here is the corresponding graphical model.
...
#### Other random effects structure

Just about any LGC you come across in the wild has only one clustering level of interest.  <span class="pack">Mplus</span>, the most common SEM tool for LGC, doesn't even provide an example in its user guide of any other situation (and it provides 18 different growth models in version 8.1).  However, it's very common to have multiple random effects structure.  In our example, these might include school, district, or other complicated structure.  Tools like <span class="pack">lme4</span> handle complicated random effects structure easily.  SEM tools do not, if at all, without resorting to the multilevel (long-format) approach, which more or less defeats the purpose of using them, as they merely mimic the standard mixed model approach, albeit with yet another and different type of syntax[^multisyntax].  However, if you have other latent variables or complicated indirect effects, this may be the way to go. 
...
[^subscript]: I'm ommiting the observation level subscript, so this can work for the single observation or entire data set.
[^multisyntax]: Honestly, for the same types of models I find the multilevel syntax of Mplus ridiculously complex relative to R packages.
