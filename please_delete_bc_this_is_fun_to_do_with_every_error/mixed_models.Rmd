...
This next depcition is commonly seen in the multilevel modeling literature.  It is shown as a two part model, one at the observation level and one at the student level.  
...
gpa_lm = lm(gpa ~ occasion, data=gpa)
summary(gpa_lm)
pander.return(summary(gpa_lm), round=3)
gpa_lm_by_group = gpa %>% 
  split(.$student) %>% 
...
An alternative approach would be to run separate regressions for every student.  However, there are many drawbacks to this- it's not easily summarized when there are many groups, typically there would be very little data within each cluster to do so, and the models are overcontextualized, meaning they ignore what students have in common.  We'll compare this result to the mixed model later.
