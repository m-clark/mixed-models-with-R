...
Another very commonly used correlation stucture is an <span class="emph">autocorrelation</span> structure, of lag order one, for the residuals.  What this means is that we assume the residuals at one time point apart correlate with some value $\rho$, observations at two time points apart correlate $\rho^2$, and so on.  As such we only need to estimate $\rho$, while the rest are then automatically determined.  Here's what it'd look like for four time points.
...
Just as generlized linear models extend the standard linear model, we can generalize (linear) mixed models to <span class="emph">generalized linear mixed models</span>.  Furthermore, there is nothing keeping us to only exponential family, as other packages would potentially allow for many other reponse distributions.
...
The fixed effects results are as expected for the attributes, with attractiveness being a very strong effect in particular.  In addition, having a partner of the same race had a positive effect while sex of the particpant was statistically negligble.

<!-- ```{r glmm_random, echo=FALSE, eval=TRUE} -->
<!-- pander(glmm_var_cor) -->
<!-- ``` -->

```{r, dependson='glmm_fixed'}
sd_model

```


For the variance components, notice there is no residual variance. This is because we are not modeling with the normal distribution for the response, thus there is no $\sigma$ to estimate.  However, the result suggests that there is quite a bit of variability from person to person.
...
Do a poisson mixed effect model using the [patent data][Data].  Predict the number of citations (`ncit`) based on whether there was opposition (`opposition`) and if it was for the biotechnology/pharmacuetical industry (`biopharm`). Use year as a random effect to account for unspecified economic conditions.  
