---
title: <span style="font-size:200%; font-variant:small-caps; ">Mixed Models in R</span>
author:  |
...
An alternate way to write the model which puts emphasis on the underlying data generating process for $y$ can be shown as follows.
...
Another way to interpret the variance output is via the <span class="emph">intraclass correlation</span>, which tells us how much of the variance is due to the clustering.  In this case it's just the student variance out of the total, or `r round(vcovs[1,1], 3)` / `r round(sum(vcovs), 3)` =  `r round(vcovs[1,1]/sum(vcovs), 2)*100`%.

##### Estimates of the random effects

What's more, we can actually get estimates of the student effects[^blup].  I show two ways for the first five students, as random effect and as random intercept (i.e. intercept + random effect).
...
The following shows the unconditional prediction vs. the conditional prediction that incorporates the random intercept for the first two students.
...
Pretty easy huh? To the left of that bar we are simply just positing a formula as we do with most modeling functions[^nointneeded]. Let's look at the results.
...
gpa %>% 
  ...
  add_lines(x=~occasion, y=~stufit, color=~going_down_now, opacity=.2, color=I(palettes$orange$complementary[2]), name='By Group') %>%
  theme_plotly()
```
...
In such a case we have observations clustered within both person and image, but person and image are not nested within one another- all participants see all 10 items.  Such a situation is typically referred to as one in which there are <span class="emph">crossed</span> random effects.  In such settings we have multiple sources variances to consider.
...
Unfortunately, <span class="pack">lme4</span> does not provide the ability to model the residual correlation structure, though many other mixed model packages do[^lmerho].  In fact, two packages that come with the basic R installation do so, <span class="pack">mgcv</span> and <span class="pack">nlme</span>.  We'll demonstrate with the latter. 
...
[^lmerho]: This feature request has been made by its users for over a decade at this point- it's not gonna happen.  The issue is that the way <span class="pack">lmer</span> works by default uses a method that won't allow it (this is why it is faster and better performing than other packages). Unfortunately the only response to this issue is 'use <span class="pack">nlme</span>'.  However many other packages work with <span class="pack">lme4</span> rather than <span class="pack">nlme</span>, and if you aren't going to use <span class="pack">lme4</span> for mixed models you might as well go Bayesian with <span class="pack">rstanarm</span> or <span class="pack">brms</span> instead of <span class="pack">nlme.</span>  I would even prefer <span class="pack">mgcv</span> to <span class="pack">nlme</span> (though it would use <span class="pack">nlme</span> under the hood) because of the other capabilities it provides, and the objects created are easier to work with in my opinion.
...
[^scalecontbin]:  Note that for a balanced binary variable, the mean `p=.5` and sd is `sqrt(p*(1-p)) = .5`
...
This section covers topics that are generally beyond the scope of what would be covered in the workshop, but may be given their section over time.
...
**Mplus** has a lot of functionality both for standard mixed models and growth curve models, though between <span class="pack">lavaan</span>, <span class="pack">mediation</span>, and the rest of R, there is little need to use Mplus except for very complicated multilevel SEM, which requires very large samples and a lot of theoretical justification, which most of the people doing SEM I come across rarely have.
