...
First we'll trim things to more recent movies to make things a little more comparable. I'll also convert 

```{r}
movies_trim = filter(movies, 
                     year > 1970  &
                     budget > 1e6 & 
                     votes > 1000) %>% 
...
Next, let's look at the relationship of time (`year`) and budget to rating. Use the lm function with the appropriate formula to predict rating and interpret the results.
...
That combined with pander will make your table publication ready without you having to format anything.
...
Add the `Action` indicator and the number of votes in thousands to the model.  Interpret the results.
